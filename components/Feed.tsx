...
export default function Feed({ items: initialItems }: { items: FeedItem[] }) {
  ...
  const visible = useMemo(() => {
    ...
    if (filter === "foryou") return items.filter(inInterests);
    return items.filter((i) => i.categories.includes(filter));
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [items, filter, interests]);
    ...
    <div className="h-[100dvh]">
      ...
      <header className="fixed inset-x-0 top-0 z-20 flex items-center justify-between bg-gradient-to-b from-bg/90 to-transparent px-6 pb-5 pt-4">
        ...
        <div className="flex items-center gap-3">
          <ThemeToggle />
          <span className="text-xs tabular-nums text-fg/40">
            ...
            {pills.map((p) => {
              ...
              return (
                ...
                >
                  ...
                  <span>{p.label}</span>
                  <span className={active ? "text-bg/50" : "text-fg/40"}>
                    {p.count}
                  </span>
                </button>
              );
      ...
      <main
        ...
        onScroll={(e) => {
          ...
          // Within 10 of the end of loaded data → fetch the next page.
          if (i >= visible.length - 10) loadMore();
        }}
